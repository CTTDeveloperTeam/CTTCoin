...
#include "config/cryptoinvest-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("cryptoinvest-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "cryptoinvest.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cryptoinvestd.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, cryptoinvest, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all CTT specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizecryptoinvestamount=<n>", strprintf(_("Keep N PIV anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
	   FormatParagraph(strprintf(_("Copyright (C) 2017-%i The CryptoInvest Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("cryptoinvest-loadblk");
...
/** Sanity checks
 *  Ensure that CryptoInvest is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CryptoInvest Core is shutting down."));
...
#endif
    // Make sure only a single CryptoInvest process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CryptoInvest Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CryptoInvest version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CryptoInvest Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart CryptoInvest Core to complete") << "\n";
    ...
    nAnonymizeCryptoInvestAmount = GetArg("-anonymizecryptoinvestamount", 0);
    if (nAnonymizeCryptoInvestAmount > 999999) nAnonymizeCryptoInvestAmount = 999999;
    if (nAnonymizeCryptoInvestAmount < 2) nAnonymizeCryptoInvestAmount = 2;
    ...
    LogPrintf("Anonymize CryptoInvest Amount %d\n", nAnonymizeCryptoInvestAmount);
    ...
    /* Denominations
       ...
       For example:
       1CTT+1000 == (.1CTT+100)*10
       10CTT+10000 == (1CTT+1000)*10
    */
